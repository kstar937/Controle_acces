...
def list_a_str(s):#Passer d'une liste de lettre à une chaine de caractères normal
    if type(s)==list:
        str1 = " "
...
def list_a_str_cd(s):#Passer d'une liste de chiffres à un chiffre complet sans espace
    if type(s)==list:
        str1 = ""
        response=str1.join(s)
    else:
    ...
    try:
        if liste[0] != '#':
            if len(liste) == 4:
                ...
                elif liste == code_public:
                    if verification_public():
                        versLCD('acces ouvert')
                        liste_commandes=[]
                        servo.write(90)
                        board.digital[8].write(1)
...
def verification_public():
    t=time.localtime()
    temps_actuel = time.strftime("%H:%M:%S",t)
    if "12:00:00"<temps_actuel<"12:10:00":
        return True
    else:
        return False
...
while True:
    ...
    if nb_refus_acces >= 3 and etat_alarme_sabotage==True:
        i=0
        while i < 5:
            buzzer.write(1)
            time.sleep(1)
